...
template <typename Value, typename... Error>
  requires(typename internal::get_unique<Error...>::type{} == type_set<Error...>{})
struct errored;

namespace internal {

template <typename Value, typename... Error>
consteval errored<Value, Error...> get_errored(type_set<Error...>);

template <typename Value, typename... Error, typename... Error2>
consteval auto get_errored(type_set<Error...> set1, type_set<Error2...> set2)
...
constexpr auto bind_errored(Errored&& value, F&& f) {
  ...
  using result =
      decltype(get_errored<typename invoke_result::value_type>(typename errored_t::error_set{}, typename invoke_result::error_set{}));
...
template <typename Value, typename... Error>
  requires(typename internal::get_unique<Error...>::type{} == type_set<Error...>{})
struct errored {
  using error_set = type_set<Error...>;
  ...
  constexpr explicit operator errored<Value, Error2...>(this Self&& self)
    requires(type_set<Error2...>{} >= error_set{})
  {
    if (self.has_value()) {
